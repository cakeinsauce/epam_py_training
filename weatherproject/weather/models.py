...
def get_city_forecaster_obj(city: str) -> Optional[ForecastOWM]:
...
def time_to_iso_format(v: int) -> Optional[datetime]:
    """Function transform time from UNIX to ISO format.
    Args:
        v: Datetime in UNIX format.
    Returns:
        Return datetime in ISO format if it's valid.
    Raises:
        ValueError: If `v` is not UNIX time.
    """
    if not str(v).isnumeric():
        print("Wrong UNIX time.")
        return None
